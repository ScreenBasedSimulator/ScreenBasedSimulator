...
public class Simulator {
	...
	public Patient simPatient() {
		...
		if (toolList.size() == 0 && prescriptionList.size() == 0) {
			...
			System.out.println(downFunctionBloodPressure());
			...
			if (prescriptionList.size() > 0) {
				...
				double currentDose = currentPrescription.getDose();
				//System.out.println(currentPrescription);

				// get prescription information
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					double resultOxygenLevel = ytFunctionOxygenLevel(currentDose * 10);
					// System.out.print("resultOL = " + resultOl);
				...
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					//
					double resultRatioForHeartRate = ytFunctionHeartRate(currentDose);
				...
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					//
					double resultRatioForBloodPressure = ytFunctionBloodPressure(currentDose);
				...
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					//
					double resultRatioForRespirationRate = ytFunctionRespirationRate(currentDose);
...
//			System.out.println("t0Function's result:" + result);
	...
	public double ytFunctionBloodPressure(double x0) {
		double result;
		if(x0 == 0.0){
			result = downFunctionBloodPressure();
		}else{
			result = 1.0 / (1 + Math.exp(-fFunction(x0)
