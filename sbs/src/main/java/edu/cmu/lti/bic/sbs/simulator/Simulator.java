...
public class Simulator {
		...
		if (toolList.size() == 0 && prescriptionList.size() == 0) {
			...
			if (currentTool.getName().equals("increase the blood pressure")) {
				...
				patient.setBloodPressure(new BloodPressure(
						currentSystolicBloodPressure, currentDiastolicBloodPressure));
			}
			...
			Prescription currentPrescription = prescriptionList.get(prescriptionList
					.size() - 1);
			...
			// the name of the tool do not correct, I just test the function
			if (currentPrescription.getDrug().getName()
					.equals("increase the heart rate")) {
				//
			...
			// the name of the tool do not correct, I just test the function
			if (currentPrescription.getDrug().getName()
					.equals("increase the blood pressure")) {
				//
			...
			// the name of the tool do not correct, I just test the function
			if (currentPrescription.getDrug().getName()
					.equals("increase the Respiration Rate")) {
				//
	...
	public void simulateWithTool(Tool eq) {
		...
		// when the engine involve the function simulateWithTool, the simulator add
		// the tool to the toolList
		toolList.add(eq);
	}
	...
	/*
	 * //the parameter is two list, toolList and drugList public Patient
	 * updatePatient(List<Tool> toolList, List<Drug> drugList){
	 * 
	 * Tool currentTool = toolList.get(toolList.size() - 1);
	 * 
	 ...
	 * 
	 * if(currentTool.getName().equals("oxygenMask")){ OxygenMask
	 * currentToolOxygenMask = (OxygenMask) currentTool;
	 * 
	 * //System.out.println("id:"+currentToolOxygenMask.getId()+" name:"+
	 * currentToolOxygenMask
	 * .getName()+" description "+currentToolOxygenMask.getDescription() +
	 * " oxygenLevel "+currentToolOxygenMask.getOxygenValue()); }
	 * 
	 * System.out.println("invoke function simWithTool");
	 * 
	 * Drug currentDrug = drugList.get(drugList.size() - 1);
	 * 
	 * System.out.println("id "+currentDrug.getId()+" name "+currentDrug.getName()+
	 * " description "+currentDrug.getDescription());
	 * 
	 * System.out.println("invoke function simWithDrugs");
	 * //simWithDrugs(drugList);
	 * 
	 * System.out.println("return patient"); return patient; }
	 */
	...
	public double downFunctionBloodPressure() {
		...
		this.patient.setBloodPressure(new BloodPressure(
				currentSystolicBloodPressure * result, currentDiastolicBloodPressure
						* result));
	...
	public double downFunctionRespirationRate() {
		...
		this.patient.setRespirationRate(new RespirationRate(result
				* currentRespirationRate));
