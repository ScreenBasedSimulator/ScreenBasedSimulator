...
public class Simulator {
	...
	public Patient initialPatient() {
		 ...
		 * patient.getRepirationRate().setRrNum(16.0); return patient;
		 */
	}
	...
	public Patient initialPatient(double systolicBloodPressure,
		...
		patient.setBloodPressure(new BloodPressure(systolicBloodPressure,
				lowerBoundDict.get("systolicBloodPressure"), upperBoundDict
						.get("systolicBloodPressure"), diastolicBloodPressure,
				lowerBoundDict.get("diastolicBloodPressure"), upperBoundDict
						.get("diastolicBloodPressure")));

		patient.setHeartRate(new HeartRate(heartRate, lowerBoundDict
				.get("heartRate"), upperBoundDict.get("heartRate")));

		patient.setOxygenLevel(new OxygenLevel(oxygenLevel, lowerBoundDict
				.get("oxygenLevel"), upperBoundDict.get("oxygenLevel")));
	...
	private void initBound() {
		...
		lowerBoundDict.put("respirationRate", 0.0);
	...
	public Simulator(Patient pt, ArrayList<Prescription> prescriptions,
		...
		if (toolList.size() == 0 && prescriptionList.size() == 0) {
			...
			if (prescriptionList.size() > 0) {
				...
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					...
					// System.out.println("resultRatioForHeartRate = " +
				...
				if (currentPrescription.getDrug().getId().equals("naloxone")) {
					...
					System.out
							.println("patient's resultRatioForRespirationRate is:"
									+ resultRatioForRespirationRate);

					double resultRespirationRate = (1 + resultRatioForRespirationRate / 5.65)
							* currentRespirationRate;

					System.out.println("patient's Respiration Rate:"
							+ resultRespirationRate);
